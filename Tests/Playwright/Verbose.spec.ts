...
test('redirects to /de with appropriate accept-language header', async ({
  browser,
}) => {
...
test('clicking word link opens Wiktionary in new tab', async ({
  page,
  context,
}) => {
  const verbosePage = new VerbosePage(page);
  ...
  await expect(newPage).toHaveURL(
    /^https:\/\/en\.(m\.)?wiktionary\.org\/wiki\/verbose$/
  );
